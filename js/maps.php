...
<?php
    $views_location = \MySql::connect()->prepare("SELECT * FROM `views`");
    $views_location->execute();
    $views_location = $views_location->fetchAll();
?>

<script type="application/json" id="google-maps-coords">
<?php
    echo '[';
    foreach($views_location as $key => $value){
        echo '{"location_name":"'.$value["ip"].'","latitude":"'.$value["lat_coord"].'","longitude":"'.$value["long_coord"].'"}';
        echo ',';
    }
    if(end($views_location)){
        echo '{"location_name":"'.$value["ip"].'","latitude":"'.$value["lat_coord"].'","longitude":"'.$value["long_coord"].'"}';
    }
    echo ']';
?>
